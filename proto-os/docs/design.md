...
- Three persistent EL0 tasks:
  ...
  - `task_b` runs `uart_server`: prints one-time `[uart] ready`, receives endpoint messages, writes payload, sends ACK reply; `FAULT_DEMO=ON` and `BENCH_MODE=RECOVERY` builds compile a one-shot fault injection (`brk #0` after the first reply), which only triggers where `task_b` actually receives traffic (MICRO)

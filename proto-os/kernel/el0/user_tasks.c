...
static const uint8_t g_ping_msg[] EL0_RODATA = "ping";
...
void __el0_task_b_entry(void) {
  ...
  for (;;) {
    uint64_t recv_len = el0_ipc_recv(EP_ECHO, recv_buf, sizeof(recv_buf));
    if (recv_len != (uint64_t)-1) {
      el0_write(g_task_b_msg, sizeof(g_task_b_msg) - 1);
      (void)el0_ipc_reply(EP_ECHO, recv_buf, recv_len);
    }
    el0_yield();
  }
}
